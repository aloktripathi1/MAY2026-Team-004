...
export function TransferAdminForm({ candidates }: { candidates: { membershipId: string; name: string; role: string }[] }) {
  ...
  return (
    <form action={formAction} className="space-y-3">
      <select name="successorMembershipId" required className="w-full rounded-xl border border-white/12 bg-white/[0.035] px-4 py-2.5 text-sm text-white outline-none transition focus:border-secondary/55">
