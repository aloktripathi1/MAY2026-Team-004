      ...
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-col items-center gap-14 md:gap-20">
          ...
          <motion.div
            ...
            className="relative mx-auto w-full overflow-hidden rounded-[22px] border border-white/10 shadow-[0_42px_140px_-60px_oklch(0.47_0.16_24_/_70%),0_26px_90px_-62px_oklch(0.78_0.13_78_/_55%)]"
          >
            <video
...
function roleRows(active: number): [string, string, string][] {
  const rows: [string, string, string][][] = [
    ...
    [["Cook-Off #42", "Count Me In open until Sunday 8 PM", "live"], ["Issue update", "Projector ticket moved to in progress", "new"], ["Clubs matched", "Based on interests", "5"]],
...
function Events({ events }: { events: LandingEvent[] }) {
  const upcoming = events.filter((event) => event.status === "upcoming").slice(0, 3);
  const totalAttendees = useMemo(() => upcoming.reduce((sum, event) => sum + event.attendeeCount, 0), [upcoming]);
      ...
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-3 md:grid-cols-3 md:items-end">
          ...
          <div className="night-panel rounded-2xl p-5">
            <div className="font-mono text-5xl font-semibold tracking-[-0.08em] text-white">{totalAttendees}</div>
            ...
            <Link key={event.id} href={`/app/events/${event.slug}`} className="group night-panel overflow-hidden rounded-2xl transition hover:border-secondary/35">
              ...
              <div className="p-5">
                ...
                <div className="mt-5 flex items-center justify-between gap-3 text-sm text-muted-foreground">
                  ...
                  <span className="font-mono text-secondary" data-event-attendance={event.id} data-attendance-count={event.attendeeCount}>{event.attendeeCount}</span>
                </div>
              </div>
            </Link>
          ))}
