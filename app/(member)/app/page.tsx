              ...
              {upcoming.map(e => (
                <Link key={e.id} href={`/app/events/${e.slug}`} className="block">
                  <GlassCard className="group flex items-center gap-5 p-4">
                    <div className="grid h-16 w-16 shrink-0 place-items-center rounded-xl border border-white/10 bg-white/[0.045] text-center">
                      <div className="text-mono-label !text-[9px]">{formatWeekday(e.date)}</div>
