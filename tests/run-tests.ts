...
const tests: TestCase[] = [
  ...
  {
    name: "parseTagInput handles null input and collapses repeated separators",
    run: () => {
      assert.deepEqual(parseTagInput(null), []);
      assert.deepEqual(parseTagInput(",,,"), []);
      assert.deepEqual(parseTagInput("  tech  ,  , ai  "), ["tech", "ai"]);
    },
  },
  {
  ...
  {
    name: "buildEventSlug falls back to an empty base when the title has no alphanumeric characters",
    run: () => {
      assert.equal(buildEventSlug("!!!", 0), "-0");
    },
  },
  {
    name: "buildEventSlug defaults to the current time when no timestamp is provided",
    run: () => {
      const slug = buildEventSlug("Design Jam");
      assert.match(slug, /^design-jam-[a-z0-9]+$/);
    },
  },
  {
  ...
  {
    name: "decideJoinRequestAction treats inactive and unset statuses consistently",
    run: () => {
  ...
  {
    name: "decideRsvpAction treats a missing going value as zero attendance",
    run: () => {
      assert.equal(
        decideRsvpAction(false, { status: "upcoming", capacity: 5, going: null, rsvpCount: 3 }),
        "register",
      );
      assert.throws(
  ...
  {
    name: "decideRsvpAction allows registration for the last available spot",
    run: () => {
      assert.equal(
        decideRsvpAction(false, { status: "upcoming", capacity: 10, going: 9, rsvpCount: 9 }),
        "register",
      );
    },
  },
  ...
  {
    name: "normalizeTaskStatus is case-sensitive and rejects blank input",
    run: () => {
      assert.throws(() => normalizeTaskStatus("Todo"), /Invalid task status/);
  ...
  {
    name: "normalizeMembershipStatus is case-sensitive and rejects blank input",
    run: () => {
      assert.throws(() => normalizeMembershipStatus("active"), /Invalid membership status/);
  ...
  {
    name: "normalizeEventApproval is case-sensitive and rejects blank input",
    run: () => {
      assert.throws(() => normalizeEventApproval("Approved"), /Invalid approval status/);
  ...
  {
    name: "requireClubAdminAccess rejects when the membership list is empty",
    run: () => {
