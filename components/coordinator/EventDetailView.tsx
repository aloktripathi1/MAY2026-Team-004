...
import { EditDetailsButton } from "@/components/coordinator/EditDetailsButton";
import { ParticipantList } from "@/components/coordinator/ParticipantList";

// Accepts the slug or the id; the clubId check below is what actually scopes it.
async function getEventForClub(slugOrId: string, clubId: string) {
...
/**
 * The Coordinator surface's single-event dashboard — extracted so both the
 * Coordinator's own page and the Admin's native /admin/events/[slug] page
 * mount the identical component instead of two copies drifting apart.
 */
export async function EventDetailView({ slugOrId, clubId }: { slugOrId: string; clubId: string }) {
  const event = await getEventForClub(slugOrId, clubId);
  if (!event) notFound();
